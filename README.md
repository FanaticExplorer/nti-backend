...
- **§4 User roles** — 8 roles: `student`, `team_leader`, `firm`, `evaluator`, `mentor`, `nti_admin`, `super_admin`, `content_editor`, enforced via `require_role()` dependency.
- **§6.1 Public web / CMS** — `content` router with CRUD for pages (`/content/pages/{slug}`) and news articles (`/content/news`), each with publish/draft status.
...
- **§6.3 Registration forms & workflow** — `applications` router implementing the full cycle: draft → submit → formal verification → evaluation → approval → onboarding → active → completed.
- **§6.4 Notifications** — Background email tasks for welcome, application submitted, status change, and organisation approved events (SMTP via `app/utils/email.py`).
- **§6.5 Dashboards & exports** — `admin/stats` endpoint with aggregation queries; `admin/export/applications` CSV export with streaming response.
...
- **§8.2 Program B — firm onboarding** — `organizations` router: firm registration, admin approval flow, member management with owner role.
- **§8.4 Program B — tracking** — Mentorship assignment, mentorship session logs, and per-application milestone tracking.
